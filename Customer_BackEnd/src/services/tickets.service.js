...
class TicketService {
    ...
    async createTicket(ticket, token) { 
        try {
            const customer = await axios.get(`http://18.209.26.169:8080/api/customers/${ticket.customerId}`,{
                httpsAgent,
                headers: {
            ...
            const allManagers = await axios.get('http://18.209.26.169:5000/api/managers', {
                httpsAgent,
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            const employeeTicketCount = await axios.get(
                `http://18.209.26.169:5000/api/managers/${manager.managerId}/collegue/ticketCount`,{
                    httpsAgent,
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
